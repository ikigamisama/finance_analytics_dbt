...
SELECT
    ...
    ROUND((AVG(customer_lifetime_value) - 1.96 * STDDEV(customer_lifetime_value) / SQRT(COUNT(*)))::numeric, 2) AS clv_ci_lower,
    ROUND((AVG(customer_lifetime_value) + 1.96 * STDDEV(customer_lifetime_value) / SQRT(COUNT(*)))::numeric, 2) AS clv_ci_upper,
    
    -- Income statistics
    ROUND(AVG(annual_income)::numeric, 2) AS mean_income,
    ROUND(STDDEV(annual_income)::numeric, 2) AS stddev_income,
    ROUND((PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY annual_income))::numeric, 2) AS median_income,
    
    -- Credit score statistics
    ROUND(AVG(credit_score)::numeric, 0) AS mean_credit_score,
    ROUND(STDDEV(credit_score)::numeric, 2) AS stddev_credit_score,
    
    -- Transaction statistics
    ROUND(AVG(transaction_count_90d)::numeric, 2) AS mean_transactions,
    ROUND(STDDEV(transaction_count_90d)::numeric, 2) AS stddev_transactions,
    
    -- Balance statistics
    ROUND(AVG(total_balance)::numeric, 2) AS mean_balance,
    ROUND(STDDEV(total_balance)::numeric, 2) AS stddev_balance,
    
    -- Churn risk statistics
    ROUND((AVG(churn_risk_score) * 100)::numeric, 2) AS mean_churn_risk_pct,
    ROUND((STDDEV(churn_risk_score) * 100)::numeric, 2) AS stddev_churn_risk_pct,
