...
SELECT
    ...
    ROUND(q3_amount - q1_amount, 2) AS iqr,
    ROUND(min_amount, 2) AS min_amount,
    ROUND(max_amount, 2) AS max_amount,
    
    -- Coefficient of variation
    ROUND(stddev_amount * 100.0 / NULLIF(mean_amount, 0), 2) AS cv_pct,
    
    -- Skewness indicator (mean vs median)
    ROUND((mean_amount - median_amount) / NULLIF(stddev_amount, 0), 2) AS skewness_indicator,
    
    -- Outlier detection bounds (1.5 * IQR method)
    ROUND(q1_amount - 1.5 * (q3_amount - q1_amount), 2) AS lower_outlier_bound,
    ROUND(q3_amount + 1.5 * (q3_amount - q1_amount), 2) AS upper_outlier_bound,
    
    CURRENT_TIMESTAMP AS last_updated
    
FROM transaction_stats
WHERE sample_size >= 100  -- Minimum for statistical validity
