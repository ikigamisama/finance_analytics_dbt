...
WITH channel_metrics AS (
    SELECT
        ...
        SUM(is_fraud_flag::int) AS fraud_count,
        SUM(is_declined_flag::int) AS declined_count,
...
SELECT
    ...
    ROUND(mean_amount::numeric, 2) AS mean_transaction_amount,
    ROUND(stddev_amount::numeric, 2) AS stddev_transaction_amount,

    -- 95% Confidence Interval
    ROUND(
        (mean_amount - 1.96 * stddev_amount / SQRT(transaction_count))::numeric,
        2
    ) AS amount_ci_lower,
    ...
    ROUND(
        (
            SQRT(
                (fraud_count * 1.0 / transaction_count)
                * (1 - fraud_count * 1.0 / transaction_count)
                / transaction_count
            ) * 1.96 * 100
        )::numeric,
        2
    ...
    -- Processing time
    ROUND(mean_processing_time::numeric, 2) AS mean_processing_ms,

    -- Engagement
    ROUND(transactions_per_customer::numeric, 2) AS avg_transactions_per_customer,
    unique_customers,
