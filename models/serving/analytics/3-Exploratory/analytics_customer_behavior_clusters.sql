...
SELECT
    ...
    ROUND(AVG(unique_categories), 1) AS avg_unique_categories,
    ROUND(AVG(num_accounts), 1) AS avg_accounts,
    ROUND(AVG(total_balance), 2) AS avg_total_balance,
    
    -- Channel preferences
    ROUND(AVG(mobile_transactions) * 100.0 / NULLIF(AVG(transaction_count), 0), 2) AS mobile_usage_pct,
    ROUND(AVG(weekend_transactions) * 100.0 / NULLIF(AVG(transaction_count), 0), 2) AS weekend_activity_pct,
    
    -- Service metrics
    ROUND(AVG(service_interactions), 1) AS avg_service_contacts,
