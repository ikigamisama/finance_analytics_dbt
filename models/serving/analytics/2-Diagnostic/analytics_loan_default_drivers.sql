...
WITH loan_performance AS (
    SELECT
        ...
        ROUND(SUM(lp.late_fee), 2) AS total_late_fees,
        
        -- Financial metrics
        AVG(lp.outstanding_balance) AS avg_outstanding_balance,
        ROUND(SUM(lp.late_payment_flag) * 100.0 / COUNT(*), 2) AS late_payment_rate,
...
SELECT
    ...
    COUNT(*) AS loan_count,
    ROUND(AVG(late_payment_rate), 2) AS avg_late_payment_rate,
    ROUND(AVG(avg_days_late), 1) AS avg_days_late,
    ROUND(AVG(total_late_fees), 2) AS avg_late_fees,
    ROUND(AVG(avg_outstanding_balance), 2) AS avg_outstanding,
