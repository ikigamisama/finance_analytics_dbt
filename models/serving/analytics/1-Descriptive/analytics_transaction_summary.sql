...
SELECT
    ...
    ROUND(SUM(transaction_amount_abs), 2) AS total_transaction_volume,
    ROUND(AVG(transaction_amount_abs), 2) AS avg_transaction_amount,
    ...
    ROUND(SUM(CASE WHEN transaction_direction = 'Debit' THEN transaction_amount_abs ELSE 0 END), 2) AS total_debits,
    ROUND(SUM(CASE WHEN transaction_direction = 'Credit' THEN transaction_amount_abs ELSE 0 END), 2) AS total_credits,
    ...
    -- Fraud Metrics
    COUNT(DISTINCT CASE WHEN is_fraud_flag = 1 THEN transaction_key END) AS fraud_transactions,
    ROUND(SUM(fraud_amount), 2) AS total_fraud_amount,
    ROUND(COUNT(DISTINCT CASE WHEN is_fraud_flag = 1 THEN transaction_key END) * 100.0 / COUNT(DISTINCT transaction_key), 2) AS fraud_rate_pct,
    
    -- High Value
    COUNT(DISTINCT CASE WHEN is_high_value_flag = 1 THEN transaction_key END) AS high_value_transactions,
