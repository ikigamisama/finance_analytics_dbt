...
SELECT
    ...
    COUNT(DISTINCT CASE WHEN t.is_fraud_flag = 1 THEN t.transaction_key END) AS fraud_count,
    ROUND(COUNT(DISTINCT CASE WHEN t.is_fraud_flag = 1 THEN t.transaction_key END) * 100.0 / COUNT(DISTINCT t.transaction_key), 2) AS fraud_rate_pct,
